...
let PROJECT_ROOT = path.resolve(
  import.meta.dirname ?? path.dirname(new URL(import.meta.url).pathname),
  ".."
);
const PLATFORM_ROOT = path.resolve(PROJECT_ROOT, "..");
...
let PROGRESS_FILE = path.join(TOOLS_DIR, "tui-progress.json");
let REVIEW_DATA_FILE = path.join(TOOLS_DIR, "review-data.json");
...
function updateDerivedPaths(): void {
  ...
  PROGRESS_FILE = path.join(TOOLS_DIR, "tui-progress.json");
  REVIEW_DATA_FILE = path.join(TOOLS_DIR, "review-data.json");
}
...
type Screen =
  | { type: "platform"; selectedIndex: number; confirmUnlock?: boolean }
...
function getCourseProgressSummary(course: PlatformCourse): CourseProgressSummary {
  ...
  const courseDir = path.join(PLATFORM_ROOT, course.directory);
  const progressFile = path.join(courseDir, "tools", "tui-progress.json");
...
function switchToCourse(courseId: string): void {
  ...
  // Pfade aktualisieren
  PROJECT_ROOT = path.join(PLATFORM_ROOT, course.directory);
  updateDerivedPaths();
  ...
  try {
    adaptiveState = loadAdaptiveState(TOOLS_DIR);
  } catch {
...
function renderPlatformScreen(): void {
  ...
  const bannerLines = [
    ...
    `${c.cyan}${c.bold}      ██║     ██╔══╝  ██╔══██║██╔══██╗██║╚██╗██║${c.reset}`,
    `${c.cyan}${c.bold}      ███████╗███████╗██║  ██║██║  ██║██║ ╚████║${c.reset}`,
    `${c.cyan}${c.bold}      ╚══════╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝${c.reset}  ${c.dim}Platform${c.reset}`,
  ];

  // Banner nur anzeigen wenn genug Platz
  if (H > 30) {
    lines.push(bEmpty(W));
    for (const bl of bannerLines) {
      lines.push(bLine(bl, W));
    }
  }
  ...
  // Zwei-Spalten-Layout
  const totalInner = W - 2;
  const leftColW = Math.max(38, Math.floor(totalInner * 0.65));
  const rightColW = Math.max(18, totalInner - leftColW - 1);

  const leftLines: string[] = [];
  const rightLines: string[] = [];

  // Linke Spalte: Kursliste
  leftLines.push(padR(`${c.bold}${c.cyan} Deine Kurse${c.reset}`, leftColW));
  leftLines.push(`${c.dim} ${"─".repeat(Math.min(leftColW - 1, 40))}${c.reset}`);
  leftLines.push("");
  ...
  for (let i = 0; i < platformConfig.courses.length; i++) {
    const course = platformConfig.courses[i];
    const prog = getCourseProgressSummary(course);
    const unlocked = isCourseUnlocked(course);
    const isSelected = i === selectedIdx;
    const courseColor = courseColors[course.color] ?? c.white;

    // Kurszeile: Marker + Icon + Name + Fortschrittsbalken
    const marker = isSelected ? `${c.cyan}${c.bold}\u25B8${c.reset}` : " ";
    const lockIcon = unlocked ? " " : "\uD83D\uDD12";
    const barWidth = Math.max(6, Math.min(12, leftColW - 45));
    const bar = fineProgressBar(prog.percent, barWidth);

    const titleMaxLen = Math.max(10, leftColW - barWidth - 25);
    const displayTitle = truncate(course.name, titleMaxLen);
    const iconStr = `${courseColor}${c.bold}${course.icon}${c.reset}`;

    leftLines.push(padR(
      ` ${marker} ${iconStr}  ${isSelected ? c.bold : ""}${displayTitle}${c.reset}` +
      `  ${bar} ${pctStr(prog.percent)}`,
      leftColW
    ));

    // Zweite Zeile: Details
    const detailParts: string[] = [];
    detailParts.push(`${prog.completedLessons}/${prog.totalLessons} Lektionen`);
    if (prog.completedLessons > 0) {
      detailParts.push(`Phase ${prog.currentPhase}`);
    }
    leftLines.push(padR(
      `       ${c.dim}${detailParts.join(" \u00B7 ")}${c.reset}`,
      leftColW
    ));

    // Dritte Zeile: Status / Letzte Lektion / Lock-Info
    if (!unlocked) {
      const prereqCourse = platformConfig.courses.find(co => co.id === course.prerequisite);
      const prereqName = prereqCourse?.name ?? course.prerequisite ?? "";
      const minPhase = course.prerequisiteMinPhase ?? 1;
      leftLines.push(padR(
        `       ${lockIcon} ${c.yellow}Abschliesse ${prereqName} Phase ${minPhase} um freizuschalten${c.reset}`,
        leftColW
      ));
    } else if (prog.lastLessonTitle) {
      leftLines.push(padR(
        `       ${c.dim}Zuletzt: ${truncate(prog.lastLessonTitle, leftColW - 20)}${c.reset}`,
        leftColW
      ));
    } else if (course.status === "planned" || prog.totalLessons === 0) {
      leftLines.push(padR(
        `       ${c.dim}Noch keine Lektionen verfuegbar${c.reset}`,
        leftColW
      ));
    } else {
      leftLines.push(padR(
        `       ${c.dim}Bereit zum Starten${c.reset}`,
        leftColW
      ));
    }

    leftLines.push(""); // Leerzeile zwischen Kursen
  }

  // Rechte Spalte: Gesamtfortschritt
  rightLines.push(`${c.bold}${c.cyan} Gesamtfortschritt${c.reset}`);
  rightLines.push(`${c.dim} ${"─".repeat(Math.min(rightColW - 1, 20))}${c.reset}`);
  rightLines.push("");
  ...
  for (const course of platformConfig.courses) {
    const prog = getCourseProgressSummary(course);
    totalCompleted += prog.completedLessons;
    totalAll += prog.totalLessons;
  }
  const overallPct = totalAll > 0 ? Math.round((totalCompleted / totalAll) * 100) : 0;
  const overallBarW = Math.max(8, rightColW - 10);
  rightLines.push(` ${fineProgressBar(overallPct, overallBarW)}`);
  rightLines.push(` ${pctStr(overallPct)} gesamt`);
  rightLines.push("");

  // Pro-Kurs Mini-Uebersicht
  for (const course of platformConfig.courses) {
    const prog = getCourseProgressSummary(course);
    const courseColor = courseColors[course.color] ?? c.white;
    const miniBarW = Math.max(4, rightColW - 12);
    rightLines.push(` ${courseColor}${course.icon}${c.reset} ${fineProgressBar(prog.percent, miniBarW)} ${pctStr(prog.percent)}`);
  }

  // Auffuellen
  const minRows = Math.max(leftLines.length, rightLines.length);
  while (leftLines.length < minRows) leftLines.push("");
  while (rightLines.length < minRows) rightLines.push("");
  ...
  if (recommendation) {
    const recCourse = platformConfig.courses.find(co => co.id === recommendation.courseId);
    if (recCourse) {
      const recInnerW = W - 6;
      lines.push(bLine(
        ` ${c.dim}\u250C\u2500 Empfehlung ${"─".repeat(Math.max(0, recInnerW - 14))}\u2510${c.reset}`,
        W
      ));
      const recText = `Mache weiter mit ${recCourse.name} \u2014 ${recommendation.lessonTitle}`;
      lines.push(bLine(
        ` ${c.dim}\u2502${c.reset} ${padR(`${c.cyan}${truncate(recText, recInnerW - 4)}${c.reset}`, recInnerW - 2)}${c.dim}\u2502${c.reset}`,
        W
      ));
      lines.push(bLine(
        ` ${c.dim}\u2502${c.reset} ${padR(`${c.dim}[Enter] Fortfahren${c.reset}`, recInnerW - 2)}${c.dim}\u2502${c.reset}`,
        W
      ));
      lines.push(bLine(
        ` ${c.dim}\u2514${"─".repeat(recInnerW)}\u2518${c.reset}`,
        W
      ));
    }
  }

  // Bestaetigungs-Dialog bei gesperrtem Kurs
  if (screen.confirmUnlock) {
    const course = platformConfig.courses[selectedIdx];
    if (course) {
      const prereqCourse = platformConfig.courses.find(co => co.id === course.prerequisite);
      const prereqName = prereqCourse?.name ?? course.prerequisite ?? "";
      const minPhase = course.prerequisiteMinPhase ?? 1;
      ...
      const confirmW = W - 6;
      lines.push(bLine(
        ` ${c.yellow}${c.bold}\u250C${"─".repeat(confirmW)}\u2510${c.reset}`,
        W
      ));
      lines.push(bLine(
        ` ${c.yellow}${c.bold}\u2502${c.reset} ${padR(`Dieser Kurs setzt ${prereqName} Phase ${minPhase} voraus.`, confirmW - 2)}${c.yellow}${c.bold}\u2502${c.reset}`,
        W
      ));
      lines.push(bLine(
        ` ${c.yellow}${c.bold}\u2502${c.reset} ${padR(`Trotzdem oeffnen? ${c.bold}[J]${c.reset} Ja  ${c.bold}[N]${c.reset} Nein`, confirmW - 2)}${c.yellow}${c.bold}\u2502${c.reset}`,
        W
      ));
      lines.push(bLine(
        ` ${c.yellow}${c.bold}\u2514${"─".repeat(confirmW)}\u2518${c.reset}`,
...
function renderCourseInfoScreen(): void {
  ...
  if (courseInfoRenderedLines.length > 0 && screen.totalLines > 0) {
    ...
    const curriculumPath = path.join(PLATFORM_ROOT, course.directory, "CURRICULUM.md");
...
function handleCourseInfoInput(key: ParsedKey): void {
  ...
  if (key.name === "escape" || key.name === "left" || key.name === "backspace") {
    ...
    currentScreen = { type: "platform", selectedIndex: Math.max(0, courseIdx) };
    renderPlatformScreen();
    return;
  }
...
function startReview(): void {
  runChildProcess(
    path.join(TOOLS_DIR, "review-runner.ts"),
    [],
    () => {
      currentScreen = { type: "main", selectedIndex: 0 };
      renderMainMenu();
...
function startExercises(lessonIndex: number): void {
  ...
  runChildProcess(
    path.join(TOOLS_DIR, "watch-runner.ts"),
    [lesson.number],
    () => {
...
function main(): void {
  ...
  currentScreen = { type: "platform", selectedIndex: Math.max(0, activeCourseIdx) };
  renderPlatformScreen();
}
