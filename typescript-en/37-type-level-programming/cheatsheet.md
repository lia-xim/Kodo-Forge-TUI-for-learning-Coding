...
2. Recursion          = loops       → type F<T> = ... F<Next> ...
3. Mapped Types       = map()       → { [K in keyof T]: ... }
```

---

## Tuple-Length Trick (Arithmetic)
...
// A | B → A & B (uses contravariant parameter position)
```
...
|---|---|
| Router parameters from URL strings | Yes — high ROI |
| Query builder with schema types | Yes — prevents real bugs |
| Config paths (get(obj, "a.b.c")) | Yes — autocomplete + safety |
| Library APIs | Yes — users benefit |
| Business logic | No — too complex, too fragile |
| "Because it's cool" | No — KISS |
...
| Limit | Value |
|---|---|
| Recursion depth (tail-recursive) | ~1000 |
| Recursion depth (non-tail-recursive) | ~50 |
| Union members | ~100,000 |
| Error message when recursion is too deep | "Type instantiation is excessively deep" |