 ...
 *
 * 5 challenges on filter without type predicate, covariance bug, spread loses tuple,
...
export const debuggingChallenges: DebuggingChallenge[] = [
  // ─── Challenge 1: filter with complex condition without type predicate ────
  {
    id: "L04-D1",
    title: "filter() with complex condition does not narrow",
    buggyCode: [
    ...
    bugLine: 3,
    options: [
      "filter() with a complex condition does not narrow the type — it requires an explicit type predicate",
      "typeof cannot be used inside arrow functions",
      "forEach does not work on filtered arrays",
      "The callback must return 'true' or 'false', not a boolean expression",
    ],
    correctOption: 0,
    hints: [
      "What type does 'strings' have after the filter?",
      "From TS 5.5 onwards, filter() infers type predicates for SIMPLE checks (e.g. v !== null). " +
        "But for COMPLEX conditions (v !== null && v.length > 0) the inference does not kick in.",
      "Fix: .filter((v): v is string => v !== null && v.length > 0)",
    ],
    fixedCode: [
      "const values: (string | null)[] = ['hello', null, '', 'world', null];",
      "",
    ...
    explanation:
      "From TypeScript 5.5 onwards, filter() can automatically infer type predicates for simple checks " +
      "(e.g. `v !== null`). BUT for more complex conditions like `v !== null && v.length > 0` " +
      "the automatic inference does not apply. You still need an explicit type predicate " +
      "((v): v is string => ...) to narrow the type.",
    concept: "type-predicate-filter",
    difficulty: 3,
  },

  // ─── Challenge 2: Array covariance bug ───────────────────────────────────
  {
    id: "L04-D2",
    ...
    options: [
      ...
      "push() does not work on assigned arrays",
      "strings[2] does not exist and returns null",
    ...
    hints: [
      ...
      "TypeScript allows the assignment string[] -> (string | number)[] " +
        "(covariance), even though it is unsound.",
      "Through 'values' you can push a number, which then also appears in 'strings'.",
    ],
    fixedCode: [
    ...
    explanation:
      ...
      "Since both variables point to the same array, you can insert a number through " +
      "'values' that then becomes visible in 'strings' as well. " +
      "This is a known soundness hole in TypeScript. Fix: use readonly " +
      "arrays or explicitly copy with spread.",
    concept: "array-covariance",
    difficulty: 4,
  },

  // ─── Challenge 3: Spread loses tuple type ─────────────────────────────────
  {
    id: "L04-D3",
    ...
    bugLine: 5,
    options: [
      "Spreading a tuple produces number[] instead of [number, number]",
      "getPoint() does not return an array",
      "Destructuring does not work with spread results",
    ...
    hints: [
      ...
      "Fix: assign directly without spread, or use 'as const'.",
    ],
    fixedCode: [
      "function getPoint(): [number, number] {",
      "  return [10, 20];",
    ...
    explanation:
      "The spread operator [...tuple] creates a new array with the type " +
      "number[] — the tuple length information is lost. This means " +
      "TypeScript reports no error for coords[2], even though the element " +
      "does not exist. Fix: assign the tuple directly (const coords = getPoint()) " +
  ...
  {
    id: "L04-D4",
    title: "Array access outside bounds",
    buggyCode: [
    ...
    bugLine: 4,
    options: [
      "Array index access always returns the element type, even for an invalid index",
    ...
    hints: [
      ...
      "Fix: enable noUncheckedIndexedAccess in tsconfig — then the " +
    ...
    explanation:
      "By default, TypeScript returns the element type for array index access " +
      "(here: string), without accounting for undefined — even when the index " +
      "is out of bounds. This is a soundness hole that can be fixed with the " +
      "tsconfig option 'noUncheckedIndexedAccess: true'. " +
      "Every index access then becomes T | undefined and requires a check.",
    concept: "unchecked-index-access",
    difficulty: 3,
  },

  // ─── Challenge 5: readonly is shallow ──────────────────────────────────
  {
    id: "L04-D5",
