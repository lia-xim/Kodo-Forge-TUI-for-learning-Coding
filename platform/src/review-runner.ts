...
const DATA_FILE = path.join(STATE_DIR, "review-data.json");
// PROJECT_ROOT wird dynamisch anhand des aktiven Kurses gesetzt
const platformFile = path.join(PLATFORM_ROOT, "platform.json");
let PROJECT_ROOT = path.join(COURSES_ROOT, "typescript");
try {
  if (fs.existsSync(platformFile)) {
    const raw = JSON.parse(fs.readFileSync(platformFile, "utf-8"));
    const active = raw.activeCourse ?? "typescript";
    const course = (raw.courses ?? []).find((c: { id: string }) => c.id === active);
    if (course?.directory) {
